...
import { EditImageModal } from '@/components/images/edit-image-modal'
...
export function EditPromptModal({ isOpen, onClose, prompt, onSuccess }: EditPromptModalProps) {
  ...
  useEffect(() => {
    if (prompt) {
      setFormData({
        ...
        model: prompt.model,
        seed: prompt.seed ? prompt.seed.toString() : '',
        width: prompt.width,
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Edit Prompt" maxWidth="lg">
